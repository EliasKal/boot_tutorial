...
; Then it sets up the stack, by assigning a value to the stack pointer, and 
; calls main.
; In order for the processor to switch to protected mode,
; a Global Descriptor Table (GDT) must be prepared.
; The GDT contains entries (descriptors) describing memory segments and their 
; read/write permissions.
; Here a flat model is adopted:
; There are two descriptors, one for the code segment and one for the data 
; segment, both having the same base and limit.
; Both segments cover the whole addressable memory.
; For the GDT to be complete, a null descriptor must be declared too.
; GDT base and limit are stored in GDTR register when lgdt instruction is 
; executed.
...
; After the switching is done, the stack is set up by assigning a proper value 
; to the esp register.
; Then main is called.
; ==============================================================================
...
global start                ; start label is declared global so that the linker
                            ; can see it
start:   
...
pmode:
    ; update segment registers to point to data segment descriptor
    mov ax, SYS_DATA_SEL
    mov ds, ax
    mov es, ax
    mov ss, ax
...
extern main                 ; main is declared extern because it is defined in 
                            ; another file
    jmp main                ; call main
